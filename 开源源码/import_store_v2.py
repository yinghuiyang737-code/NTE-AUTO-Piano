...
class ImportStore:
    ...
    def import_file(self,source):
        ...
        if existing:return existing["midi"],existing.get("media"),existing["display_name"]
        if source.suffix.lower()==".zip":midi,media=self._import_zip(source)
        elif source.suffix.lower() in MIDI_EXTS:
            folder=self.library/uuid.uuid4().hex;folder.mkdir(parents=True)
            midi=folder/source.name;shutil.copy2(source,midi);media=None
        else:raise ValueError("仅支持 MIDI 或包含 MIDI 的 ZIP")
        item={"id":uuid.uuid4().hex,"source_hash":source_hash,"midi":str(midi),
              "media":str(media) if media else None,"display_name":source.name,
              "mode":None,"created":time.time()}
        items=self._read_index();items.append(item);self._write_index(items)
        return item["midi"],item["media"],item["display_name"]
    ...
    def _import_zip(self,source):
        ...
        with zipfile.ZipFile(source) as archive:
            for info in archive.infolist():
                ...
                if suffix not in MIDI_EXTS|MEDIA_EXTS:continue
                name=Path(info.filename).name
                if not name:continue
                target=folder/name
                with archive.open(info) as src,target.open("wb") as dst:shutil.copyfileobj(src,dst)
                (midis if suffix in MIDI_EXTS else media).append(target)
        if not midis:
        ...
        return midis[0],media[0] if media else None

    def save(self,midi,media,display_name,mode):
        data={"midi":midi,"media":media,"display_name":display_name,"mode":mode}
        ...
        for item in items:
            if item.get("midi")==midi:
                item.update({"media":media,"display_name":display_name,"mode":mode});found=True;break
    ...
    def load(self):
        try:
            ...
            if data.get("media") and not Path(data["media"]).exists():data["media"]=None
            return data
